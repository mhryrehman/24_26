...
public class ResetCredentialSelectUser implements Authenticator, AuthenticatorFactory {
    ...
    public static final String PROVIDER_ID = "validate-user-exists";
    ...
    public void authenticate(AuthenticationFlowContext context) {
        ...
        if (existingUserId != null) {
            ...
            logger.infof("kcId=%s Forget-password triggered when authenticating user after first broker login. Prefilling reset-credential-choose-user screen with user '%s' ", existingUser.getId(), existingUser.getUsername());
    ...
    public void action(AuthenticationFlowContext context) {
        ...
        if (username == null || username.isEmpty()) {
            ...
            event.detail(Constants.FAILURE_DETAILS, Constants.MISSING_USERNAME).error(Errors.USERNAME_MISSING);
        ...
        if (user == null) {
            ...
            event.detail(Constants.CLIENT_IP, Utils.getClientIpFromAuthSession(context.getAuthenticationSession())).detail(Constants.FAILURE_DETAILS, Constants.USER_NOT_FOUND).error(Errors.USERNAME_MISSING);
            Response challenge = context.form()
            ...
            event.clone().detail(Constants.CLIENT_IP, Utils.getClientIpFromAuthSession(context.getAuthenticationSession())).detail(Details.USERNAME, username).user(user).error(Errors.USER_DISABLED);
            context.clearUser();
    ...
    public String getDisplayType() {
        return "Choose User (with user validation)";
    }
    ...
    public String getHelpText() {
        return "Choose a user during authentication. It will return error if user did not exist in keycloak.";
    }
