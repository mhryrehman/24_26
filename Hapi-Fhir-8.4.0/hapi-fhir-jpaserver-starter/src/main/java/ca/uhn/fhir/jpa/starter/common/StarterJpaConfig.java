...
import ca.uhn.fhir.jpa.starter.curemd.CustomRestfulServer;
import ca.uhn.fhir.jpa.starter.curemd.customproviders.CustomPatientResourceProvider;
import ca.uhn.fhir.jpa.starter.curemd.export.BulkDataExportProviderYasir;
import ca.uhn.fhir.jpa.starter.curemd.interceptors.*;
import ca.uhn.fhir.jpa.starter.curemd.provider.ExportCancelOperationProvider;
...
public class StarterJpaConfig {
	...
	public RestfulServer restfulServer(
		...
		BulkDataExportProviderYasir bulkDataExportProvider,
		...
		fhirServer.registerInterceptor(new GranularScopeEnforcerInterceptor());
		fhirServer.registerInterceptor(new CustomPartitionInterceptor());
		fhirServer.registerInterceptor(new CustomErrorResponseInterceptor());
		fhirServer.registerInterceptor(new CapabilityStatementCustomizer());
		fhirServer.registerInterceptor(new ValidationResultOverrideInterceptor());
		fhirServer.registerProvider(new ExportCancelOperationProvider(appContext.getBean(JdbcTemplate.class)));
//		fhirServer.registerProvider(new CustomPatientResourceProvider());
